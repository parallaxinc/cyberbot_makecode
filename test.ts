...
// Read Pins 7-4 and store their binary states in a single value, then check the binary state or each bit starting at bit 3 and ending at bit 0, then store it in a variable and scroll the states across the display
let qti = 0
let bits = ""
basic.forever(function () {
...
// Read the received long signal, change it to a dictionary, save the values from the dict in separate variables, and display each input that was received for 5 seconds
...
// Display a request for an input of your favorite animal on a terminal, wait for the input to be received with a carriage return, enter a new line on the serial, set everything entered to complete upper case, display the inputted string on the same terminal showing it is all upper case.
